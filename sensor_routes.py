...
class SensorPayload(BaseModel):
    ...
    timestamp: datetime
    sensor_data: List[SensorEntry]


class IoTSystemCreate(BaseModel):
    name: str
    password: str
...
@router.post("/iot_systems")
def create_iot_system(
    payload: IoTSystemCreate,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    existing = db.query(IoTSystem).filter(IoTSystem.name == payload.name).first()
    if existing:
        raise HTTPException(status_code=400, detail="System name already exists")
    system = IoTSystem(
        name=payload.name, password_hash=get_password_hash(payload.password)
    )
    db.add(system)
    db.commit()
    db.refresh(system)
    link = UserIoTLink(user_id=current_user.id, iot_system_id=system.id)
    db.add(link)
    db.commit()
...
@router.post("/data")
def receive_data(payload: SensorPayload, db: Session = Depends(get_db)):
    system = db.query(IoTSystem).filter(IoTSystem.name == payload.iot_system_name).first()
    if not system or not verify_password(payload.iot_system_password, system.password_hash):
        raise HTTPException(status_code=401, detail="Invalid IoT system")

    for entry in payload.sensor_data:
